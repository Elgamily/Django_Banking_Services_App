...
def getBalance(user):
    ...
    total_deposits = History.objects.filter(user=user, status='success', type='deposit').aggregate(total=Sum('amount'))['total'] or 0.0
    
    total_withdrawals = History.objects.filter(user=user, status='success', type='withdraw').aggregate(total=Sum('amount'))['total'] or 0.0
    
    balance = float(total_deposits) - abs(float(total_withdrawals))
...
class BalanceOperationsView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        # status - if the amount on the account is not enough when attempting to withdraw funds, 
        # the status is failure, otherwise withdraw
        ...
        if amount is None or type not in ['withdraw', 'deposit']:
...
class ViewTransactionHistoryView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        # '''
        transactions = History.objects.filter(user=self.request.user)
...
class CurrencyExchangeView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        #     If the amount value from the form is converted to float type, then write the amount value from 
        #     the form converted to float to the amount variable. Otherwise, write None.
        #     2) add a currency variable that contains the currency value from the form.
        #     3) if the variables data or amount contain None, return page with empty context (empty_context). 
        #     Otherwise, perform the following steps
        #     4) generate the exchange_rate variable by calculating the corresponding value from 
        #     the data variable
        #     5) generate the exchanged_amount variable, 
        #     which contains the converted currency to two decimal places.
